...
.PARAMETER Path
    Path to a .vbe file. The file content is read and decoded automatically.
...
.NOTES
    ...
    Also included in adPEAS v2: https://github.com/61106960/adPEAS
#>

function ConvertFrom-VBE {
    [CmdletBinding(DefaultParameterSetName = 'ByString')]
    param(
        [Parameter(Mandatory=$true, ParameterSetName='ByString', ValueFromPipeline=$true)]
    ...
    process {
        ...
        if ($PSCmdlet.ParameterSetName -eq 'ByPath') {
            $EncodedScript = Get-Content -Path $Path -Raw
        }
        ...
        try {
            ...
            if ($EncodedScript -match '#@~\^.{6}==(.+).{6}==\^#~@') {
                ...
                $processedData = $processedData -replace '@&', "`n"    # Line feed
                $processedData = $processedData -replace '@#', "`r"    # Carriage return
                ...
                foreach ($char in $processedData.ToCharArray()) {
                    ...
                    # Only increment index for bytes < 128
                    if ($byteValue -lt 128) {
                        $index++
                    }

                    # Decode printable characters (except < > @)
                ...
                Write-Warning "Not a valid VBE encoded script (missing #@~^ markers)"
                return $EncodedScript
            }

        } catch {
            Write-Error "VBE decoding failed: $_"
        }
    }
}
